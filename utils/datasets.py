...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        # Define labels
        self.label_files = [x.replace('images', 'txt').replace(os.path.splitext(x)[-1], '.txt')
                            for x in self.img_files]     #这里有修改！！！
                    ...
                    if extract_bounding_boxes:
                        ...
                        for j, x in enumerate(l):
                            ...
                            if not os.path.exists(Path(f).parent):
                                os.makedirs(Path(f).parent)  # make new output1 folder
...
def load_mosaic(self, index):
    ...
    #     label_=label.tolist()
    #     for label in label_:
    #         if (label[0] == 0.0):color = (0,255,0)
    #         elif (label[0] == 1.0):color = (255,0,0)
    #         elif (label[0] == 2.0):color = (0, 0, 255)
    #         elif (label[0] == 3.0):color = (0, 255, 255)
    #         elif (label[0] == 4.0):color = (255, 0, 255)
    #         # aa= abs(int(label[0][2]))
    #         cv2.rectangle(img4, (abs(int(label[1])),abs(int(label[2]))), (abs(int(label[3])),abs(int(label[4]))), color)
