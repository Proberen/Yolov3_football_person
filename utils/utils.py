...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    # Define criteria 结合sigmoid的二分类交叉熵
    BCEcls = nn.BCEWithLogitsLoss(pos_weight=ft([h['cls_pw']]), reduction=red)
    BCEobj = nn.BCEWithLogitsLoss(pos_weight=ft([h['obj_pw']]), reduction=red)

    # class label smoothing https://arxiv.org/pdf/1902.04103.pdf eqn 3
    cp, cn = smooth_BCE(eps=0.0)  #eps给0相当于根本就没有使用这个技巧
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if nb:  # number of targets
            ...
            # ps是预测值 ！！！ ps[:,0:2]->xy    ps[:,2:4]->wh   ps[:,4]->obj    ps[:,5:]->class
            # GIoU
            ...
            tobj[b, a, gj, gi] = (1.0 - model.gr) + model.gr * giou.detach().clamp(0).type(tobj.dtype)  # giou ratio     model.gr=0所以，其实这个函数没什么用
...
def build_targets(model, targets):
    # targets = [image_batch, class, x, y, w, h]

    nt = len(targets) #39
    tcls, tbox, indices, av = [], [], [], []
    multi_gpu = type(model) in (nn.parallel.DataParallel, nn.parallel.DistributedDataParallel)
    reject, use_all_anchors = True, True
    for i in model.yolo_layers:  #这里进入一个循环 2层  13*13 和 26*26
        # get number of grid points and anchor vec for this yolo layer
        if multi_gpu:
            ng, anchor_vec = model.module.module_list[i].ng, model.module.module_list[i].anchor_vec
        else:
            ng, anchor_vec = model.module_list[i].ng, model.module_list[i].anchor_vec #第一个循环ng为[13 13] anchor_vec为tensor([[ 2.53125,  2.56250],[ 4.21875,  5.28125],[10.75000,  9.96875]], device='cuda:0')对应锚点下采样相同的倍数
            # 416/13 = 32  anchor对应81,82,  135,169,  344,319  都除以13
        # iou of targets-anchors
        t, a = targets, []
        gwh = t[:, 4:6] * ng  #ng为[13,13] 其中gwh中只有宽和高
        if nt: #39
            iou = wh_iou(anchor_vec, gwh) #shap 为 [3,39]  真实39个框 预设了3个anchor，所以相当于真实图像中的39个框（不考虑坐标只考虑wh）每个框与3个anchor（不考虑坐标只考虑wh）都进行iou
...
def coco_single_class_labels(path='../coco/labels/train2014/', label_class=43):
    ...
    if os.path.exists('new/'):
        shutil.rmtree('new/')  # delete output1 folder
    os.makedirs('new/')  # make new output1 folder
...
def print_mutation(hyp, results, bucket=''):
    ...
    with open('evolve.txt', 'a') as f:  # append train_png
        f.write(c + b + '\n')
...
def plot_evolution_results(hyp):  # from utils.utils import *; plot_evolution_results(hyp)
    ...
    for i, (k, v) in enumerate(hyp.items()):
        y = x[:, i + 7]
        # mu = (y * weights).sum() / weights.sum()  # best weighted train_png
        mu = y[f.argmax()]  # best single train_png
        plt.subplot(4, 5, i + 1)
